 ...
 * AND (target :Post OR target :Comment)  - only like posts or comments
...
const Like = async ({
  ...
  await RunCypherQuery(`MATCH (user:User), (target) WHERE ID(user) = ${logged} AND ID(target) = ${target} AND (target:Post OR target:Comment)
